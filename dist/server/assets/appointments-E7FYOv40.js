...
function Appointments() {
  ...
  const getSinglePatientRecord = async (userId) => {
    const {
      data,
      error
    } = await supabase.from("patients").select("id, full_name").eq("user_id", userId).order("created_at", {
      ascending: true
    }).limit(1);
  ...
  reactExports.useEffect(() => {
    ...
    if (isStaff) {
      ...
      getSinglePatientRecord(user.id).then((data) => {
        if (data) {
          setPatients([data]);
